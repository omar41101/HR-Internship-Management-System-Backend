...
/**
 * WHAT: SpecialShift Model
 * WHY: Stores reusable special shift types that admins can define once and
 *      apply to multiple timetable slots. Separate from one-time custom shifts
 *      which are stored inline in the Timetable document (specialShiftData).
 *
 * Three layers in the system:
 *   1. Reusable types (this model) → referenced via specialShiftId in Timetable
 *   2. Custom one-time shifts → stored inline as specialShiftData in Timetable
 *   3. Standard shifts (Morning/Evening/Full-time/Day Off) → unchanged
 */
